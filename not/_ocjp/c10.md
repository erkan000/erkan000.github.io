...
Bir uygulama bir veya daha fazla "process" çalıştırabilir. Her process'in kendi çalışma alanı ve kaynakları var gibi düşünebiliriz. Her bir process de birden fazla thread(lightweight process) çalıştırabilir. Çogu JVM tek bir process de çalışır. Process içindeki thread'ler ise bu process'in kaynaklarını paylaşarak kullanabilir. Yalnız bu paylaşım beraberinde başka sorunlarıda getirir. Operasyonların karışması(*thread interference*) veya çelişkili nesne değerleri(*memory inconsistency*) oluşabilir. Bunları engellemek için veriler senkronize edilebilir ama bu da *deadlock*, *starvation*, ve *livelock* gibi sorunlara yol açar.

Multithreading büyük bir konudur. Sınavda bazı önemli kısımları sorulmaktadır. Önemli olan Thread'ler tarafından neyin garanti edildiğini ve neyin garanti edilmediğini kavramaktır. Garanti edilmenin dışında;

*Expected 		Beklenen, umulan*

P*robable 		Muhmemel, olası*

A*ssured 		Emin olmak*

## Create and use threads

Thread sınıfı ve Runnable interface'i ile yeni thread'ler oluşturulur. JVM multithreading'i destekler. Bu da altındaki işlemcinin multithreading'i çeşitli yöntemlerle desteklemesinden ötürü oluşur.

Java dünyasında sıklıkla "Thread instance" ve "a thread of execution" duymuşsunuzdur. Bunlar aynı şey değildirler. İlki bir java nesnesidir. Java threadlerinin implementasyonu JVM specific'dir. Thread oluşturma işi JVM tarafından OS 'a gönderilir. Gerisini İngilizce yazıyorum;
...
![](media/image122.png){width="5.090277777777778in"
...
Uygulamaızda "Thread of execution" başlayınca, Thread instance'daki run metodunu çağırır. Böylece start metodu yeni thred oluşmasını tetikleyecek ve ona yine kaynaklar atanmasını sağlayacaktır.

Örnek kodumuz chapter10.s1.HelloThread package'ı içinde bulunur. Buradaki en önemli husus Dancing mi yoksa singing mi önce yazılır hiçbir zaman bilemeyiz. Tamamen OS ve JVM'e bağlıdır. Burada Thread sınıfının start metodunu çağırıyoruz. Peki bu metodu biz Sing sınıfımızda override etteydik ne olurdu? Normal metod gibi çalışırdı, hiçbir zaman Thread oluşmazdı!
...
Bir thread'in birçok durumu vardır. Thread nesnesini instante edince hemen başlamaz. Sınavda aşağıdaki gibi durumları ve bu durumlardan geçiş yapmayı sağlayan metodları bilmek kafidir. Thread içindeki durumları tutan Enum vardır, bu durumlar şunlardır;

![](media/image123.png){width="5.25in" height="2.5416666666666665in"}

Aşağıdaki şekil Thread'in durumlarını göstermektedir.

![](media/image124.png){width="5.263888888888889in"
...
Çalışan bir thread içinde sleep(int), join(int), veya wait(int) metodları çağrılır ise thread TIMED\_WAITING durumuna geçer. Bu durumda thread durur ve belirlenen zaman sonunda çalışmasına devam eder.(Thread schuler ne zaman uygun görür ise)
...
Sınavda NEW durumunda join çağrılabilir mi, bir thread de start 2 kez çağrılırsa ne olur, gibi sorular çıkması büyük ihtimaldir.

### Methods of class Thread

Thread sınıfı metodları şöyle sınıflandırılabilir.

![](media/image125.png){width="5.208333333333333in" height="3.0625in"}

### Start thread execution

Bir thread instance tanımlandıktan sonra NEW konumunda olur. Bu konumda iken start metodu çağrılır ise Thread RUNNABLE konumuna geçer. Start'ı diğer bütün durumlarda çağırır isek IllegalThreadStateException hatası
alırız. Mesela yeni tanımlanan bir thread'i 2 defa start eder isek 2. de IllegalThreadStateException fırlatılır.

Bunun 2 örneği chapter10.s2ThreadStart sınfında var. İlk örnekte start ile başladığı için Thread RUNNABLE konumuna geçer ve ikinci start hata verir. Konsola yazabilirde yazmayabilirde çünkü işlemci çok hızlı davranıp main'de 2. Komutu çalıştırabilir. İkinci örnekte ise doğrudan run metodu çağrıldığı için yeni bir thread of execution oluşturulmaz, normal bir metod gibi çalışır.
...
Bir kod test ederken birden fazla thread çalıştığını düşünelim. Aynı durumu yaratmak için threadlerden birini suspend etmek isteyebiliriz. Bu durumda Thread sınıfının statik metodu olan yield çalıştırılabilir. Ama bu metod thread'in suspend olacağını asla garanti etmez. Schuler'a sadece bir tavsiyedir, bunu ignore bile edebilir. Bu metod kodun herhangi bir yerinden çağrılabilir. Bu metod [statiktir] ve [hiçbir hata fırlatmaz]. Örneği chapter10.s2.Yield

![](media/image126.png){width="4.826388888888889in"
height="1.9583333333333333in"}

Bu metod peki neyi garanti eder, hiçbirşeyi garanti etmez!

**METHOD THREAD.SLEEP(time)**

Statik metod Thread.sleep() bir threadin çalışmasını en azından belirtilen zaman kadar durmasını garanti eder ve READY state e geçirir. Mesela ekrandaki topun hareketini yavaşlatmak için kullanılabilir. Örneği kodda ThreadSleep sınıfıdır.
...
Bir thread kendini başka bir thread'in işinin bitmesi için bekletebilir. Bunu join() metodu ile yapar. Mesala uygulama geliştirme takımı ekran dizaynı bitmeden kodlama işine geçemez, bu sebepten dizayn takımını beklemek zorundadır.

Join metodu [InterruptedException] hatası fırlatır! Örnek kod; chapter10.s2.ThreadJoin Örneğin muhtemel çıktıları aşağıdadır. Join metodu diğer thread işini bitirince çalışmasına kaldığı yerden devam eder!
...
![](media/image127.png){width="5.3125in" height="1.1111111111111112in"}
...
Yukardaki metodlar da çağıran threadin en az bu süreler kadar bekleyeceğini garanti eder.(Tabii thread sonlandırılmazsa) join aslında arka planda wait(), isAlive() ve notifyAll() metodlarını kullanır.

**METHODS WAIT(), NOTIFY(), AND NOTIFYALL()**

Bir server'ın istekleri karşılaması gibi bir thread de bir nesneyi bekleyebilir. Wait() metodu bir nesneyi notify veya notifyAll
çalıştırılana kadar bekleyebilir. Bu metodlar bütün java nesneleri üzerinde çalıştırılabilir çünkü bu metodlar Object sınıfının üyeleridir. Bu metodlar senkron metodlardan ve nesne monitörü olan thread'lerden çağrılabilir.
...
Thread'ler memory'nin bazı kısımlarını paylaşan hafif process'lerdir. Bu threadleri verimli yapar ama yanında memory sorunları da getirir. Hafıza çeşitli tekniklerle threadler tarafından paylaşılarak kullanılabilir. Bunlar;
...
- [Method area], Class bilgileri vardır, mesela değişkenler ve metodlar gibi. Burada static değişkenler de yer alır.
  Bu statik değişkenlere JVM içindeki bütün threadlerden ulaşılabilir. Bu değişkenleri "class loader" tarafından paylaştırılır. Her class loader kendi statik değişkenlerine sahiptir.
...
![](media/image128.png){width="5.347222222222222in"
...
![](media/image129.png){width="5.298611111111111in"
...
Bir metoda Syncronized keyword ü eklenir ise, o metodu aynı anda sadece bir thread çalıştırabilir. Eğer bir thread syncronized bir metodu çalıştırmak isterse metodun olduğu nesnenin monitörünü elde etmesi gerekir. Mönitörü elde eder ise(lock) başka hiçbir thread [o nesnedeki hiçbir syncronized metodu] çalıştıramaz. Kitlendikten, elde ettikten sonra bir thread 3 şekilde monitör üzerindeki kiliti bırakır. Çalışması tamamlanınca, başka bir işlemin bitmesini beklemesi gerekiyorsa veya çalışırken bir hata fırlatır ise. Bu durum aşağıdaki şekilde görülmektedir. Bir thread bekleme durumuna yield veya wait metodlarını kullanrak girer. Tekrar monitörü notify veya notifyAll metodları çağrılınca alabilir.

![](media/image130.png){width="5.256944444444445in"
height="1.6527777777777777in"}

**SYNCHRONIZED METHODS**

Bi metod, tanımına syncronized keyword'ü eklenerek synronized metod olur. İnstance ve statik metodların ikiside syncronized olabilir. Bir thread synchronized metod çağırır ise onun monitörüne otomatik olarak sahiplenir. Çağrılan metod instance metod ise thread monitör çağrıldığı sınıfın instance'ına kitlenir. (this) Çağrılan metod statik metod ise
çağrıldığı sınıfı temsil eden Class nesnesine kilitlenir. (For nonstatic instance synchronized methods, a thread locks the monitor of the object on which the synchronized method is called. To execute static synchronized methods, a thread locks the monitor associated with the Class object of its class.)

Bir önceki karışan örnekteki metodları syncronized yaparak sorunu ortadan kaldırırız.

Synronized keyword'ü datanın değiştiği yere konması gerekmektedir. Yoksa işe yaramaz.
...
Concurrent erişimde değişken değelerinin güvende tutmak için bir diğer yol ise volatile değişkenler kullanmaktır.Syncronied bloklara göre dezavantajlıdırlar. Atomik işlemleri desteklemezler. Kullanmamızın tek
amacı basitlik ve bazı durumlarda daha iyi performans.
...
![](media/image131.png){width="5.229166666666667in" height="2.375in"}
...
Birbirine bağlı 2 thread düşünelim. Eğer birinin işi bitmeden diğeri değer okumaya kalkarsa yanlışlık olabilir. bu durumda bunların birbiriyle işinin bittiğini haberleşmesi lazımdır. Mesela bir arkadaşınız ile dışarı çıkacaksınız. Evde bekleyen arkadaş her dakika dışarı çıkıp bakması zor bir durumdur. Onun yerine karşıdaki kişi gelince haber verebilir ve dışarı çıkabilirler. Java da bu haber verme işi wait ve notify metodları ile yapılır. Bu metodlar Object sınıfında tanımlandığı için her Java nesnesi üzeinde çağrılabilir!

Wait, notify ve notifyAll metodların birbiri ile haberleşmesi için kullanılır ve bunlar Thread sınıfının değil Object sınıfının
üyeleridir!!!

Bir thread wait ve notify metodlarını çağırmak için nesnesinin monitörüne kitlenmesi gereklidir. Ve bu metodları çağırma işi syncronized metod veya bloklardan yapılmalıdır. Örneği chapter10.s4.ThreadCommunication sınıfında vardır. Bu sınıf
çalıştırıldığında GoRafting thread'i başlar. Bu thread içindeki run metodunda synchronized(friend) satırı bulunur. Bu satır friend nesnesine kitlenir ve ardından friend nesnesi üzerinde wait çalıştırır. Kod burada durur. Kod 2 şekilde devam eder(kilit bıraklılır);
...
Thead\'in join metodu başka bir threadin iş yapmasını bekler, fakat wait ve notify beklemez. wait\'i çağıran thread sadece, notify çağrılmasını bekler.

Sınav bu metodlar çağrıldığında thread durumlarının ne olacağını sorabilir. Bunlar aşağıda gösterilmiştir.

![](media/image132.png){width="5.173611111111111in" height="1.8125in"}
...
![](media/image133.png){width="4.666666666666667in"
